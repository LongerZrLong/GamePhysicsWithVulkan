...
int GetTimeMicroseconds() {
	...
	auto deltaTime = std::chrono::high_resolution_clock::now() - gStartTime;

	return std::chrono::duration_cast<std::chrono::microseconds>(deltaTime).count();
}
...
void Application::OnMouseMoved( GLFWwindow * window, double x, double y ) {
	Application * application = reinterpret_cast< Application * >( glfwGetWindowUserPointer( window ) );
	auto state = glfwGetMouseButton(window, 0);  // Check if left button pressed
	application->MouseMoved( (float)x, (float)y, state == GLFW_PRESS);
...
====================================================
*/
void Application::MouseMoved( float x, float y , bool pressed) {
    if (!pressed) {
        m_mousePosition = Vec2(x, y);
        return;
    }
	Vec2 newPosition = Vec2( x, y );
	Vec2 ds = m_mousePosition - newPosition;
